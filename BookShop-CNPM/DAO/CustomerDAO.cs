...
namespace BookShop_CNPM.DAO
{
    public class CustomerDAO : IDAO<CustomerDTO>
    {
        ...
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery("select * from khachhang WHERE LOWER(soDienThoai)=LOWER(@soDienThoai) and hienThi=1;",
                new MySqlParameter[] {
        ...
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery("select * from khachhang WHERE LOWER(soDienThoai)=LOWER(@soDienThoai) and maKhachHang!=@id and hienThi=1;",
                new MySqlParameter[] {
                    new MySqlParameter("@soDienThoai", value.Trim().ToLower()),
        ...
        public List<CustomerDTO> Search(string searchInput)
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery(
                    "SELECT * FROM khachhang WHERE (soDienThoai LIKE @SoDienThoai OR maKhachHang = @MaKhachHang OR tenKhachHang LIKE @TenKhachHang) and hienThi=1;",
                    new MySqlParameter[] {
        ...
        public List<CustomerDTO> loadCustomerChartData()
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery(
                ...
                "FROM khachhang WHERE hienThi=1 " +
                "ORDER BY daMua DESC " +
                "LIMIT 5;"
            );
            if (dataTable.Rows.Count <= 0) return null;
        ...
        {
            string sql = $@"UPDATE khachhang SET hienThi = 0 WHERE maKhachHang=@maKhachHang;";

            int rowChanged = DataProvider.Instance.ExecuteNonQuery(sql,
                new MySqlParameter[] {
                    new MySqlParameter("@maKhachHang", id),
                });
