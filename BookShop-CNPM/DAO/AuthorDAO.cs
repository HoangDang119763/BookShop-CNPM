...
namespace BookShop_CNPM.DAO
{
    public class AuthorDAO : IDAO<AuthorDTO>
    {
        ...
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery("select * from tacgia WHERE LOWER(tenTacGia)=LOWER(@tenTacGia) and hienThi=1;",
                new MySqlParameter[] {
        ...
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery("select * from tacgia WHERE LOWER(tenTacGia)=LOWER(@tenTacGia) and maTacGia!=@id and hienThi=1;",
                new MySqlParameter[] {
                    new MySqlParameter("@tenTacGia", value.Trim().ToLower()),
