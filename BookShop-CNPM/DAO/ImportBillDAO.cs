...
namespace BookShop_CNPM.DAO
{
    public class ImportBillDAO : IDAO<ImportBillDTO>
    {
        ...
        public bool createImportBillDetail(ImportBillDetailDTO data, int phanTramLoiNhuan)
        {
            ...
            decimal giaNhap = data.DonGia - Convert.ToDecimal(Convert.ToDecimal(data.DonGia * Convert.ToDecimal(phanTramLoiNhuan / 100.0)).ToString().Split('.')[0]);
        ...
        public bool update(ImportBillDTO data)
        {
            string sql = $@"UPDATE phieunhap SET 
                            ...
                            tongTien=@tongTien, @phanTramLoiNhuan WHERE maDonNhapHang=@maDonNhapHang;";
