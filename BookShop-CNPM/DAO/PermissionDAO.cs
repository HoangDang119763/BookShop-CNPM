...
namespace BookShop_CNPM.DAO
{
    public class PermissionDAO : IDAO<PermissionDTO>
    {
        ...
        public DataTable getAll() {
            return DataProvider.Instance.ExecuteQuery("select * from quyenhan where hienThi=1;");
        }

        public bool checkDuplicateName(string value)
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery("select * from quyenhan WHERE LOWER(tenQuyenHan)=LOWER(@tenQuyenHan) and hienThi=1;",
                new MySqlParameter[] {
        ...
        {
            DataTable dataTable = DataProvider.Instance.ExecuteQuery("select * from quyenhan WHERE LOWER(tenQuyenHan)=LOWER(@tenQuyenHan) and maQuyenHan!=@id and hienThi=1;",
                new MySqlParameter[] {
                    new MySqlParameter("@tenQuyenHan", value.Trim().ToLower()),
        ...
        public DataTable searchData(string value)
        {
            string sql = $@"SELECT * FROM quyenhan 
                            WHERE (maQuyenHan LIKE @maQuyenHan OR tenQuyenHan LIKE @tenQuyenHan) and hienThi=1;";
