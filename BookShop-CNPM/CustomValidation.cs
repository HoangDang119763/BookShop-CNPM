        ...
        public bool checkTextbox(Guna2TextBox txt, Label errMsgLbl, Panel line, string[] rules)
        {
            foreach (string rule in rules)
            {
                switch (rule)
                {
                    ...
                    /*Hoang----------------------------------*/
                    case "valid-date":
                        if (!checkTextboxEmptyWithMsg(txt, "Ngày lập của hóa đơn phải ít hơn 7 ngày!", errMsgLbl, line))
                        {
                            return false;
                        }
                        break;
                    ...
                    case "email":
                        ...
                        break;
                    case "phone-number":
                        if (!checkTextboxWithRegex(txt, new Regex("^0(3[2-9]|5[6|8|9]|7[0|6-9]|8[0-6|8|9]|9[0-4|6-9]|28)[0-9]{7}$"), "Trường này phải là một số điện thoại", errMsgLbl, line))
                        {
                            return false;
                        }
                        break;
