...
namespace BookShop_CNPM.GUI.Manager
{
    public partial class ProductStatisticGUI : Form
	{
		...
		private void loadChartView()
		{
			try
			{
                ...
                if (numberBookSoldDT != null)
                {
                    ...
                    maxValRounded = (int)Math.Ceiling((decimal)maxVal / 10) * 10;
                }
				...
				foreach (int m in months)
				{
					if (numberBookSoldDT != null)
					{
						...
						foreach (DataRow row in numberBookSoldDT.Rows)
						{
							...
							{
								found = true;
								chartVals.Add(Convert.ToInt32(row["soLuong"]) / (decimal)10);
								break;
							}
						}
						if (!found)
			...
			try
			{
				...
				if (bookList != null)
				{
					foreach (BookDTO book in bookList)
					{
						...
						int daDoi = CustomerChangeBillBUS.Instance.getChangeQuantityByBookId(book.MaSach.ToString());

						int daTra = CustomerRefundBillBUS.Instance.getRefundQuantityByBookId(book.MaSach.ToString());

						decimal doanhThu = CustomerBillBUS.Instance.getRevenue(book.MaSach.ToString());

						decimal tienDaDoi = CustomerChangeBillBUS.Instance.getChangeRevenueByBookId(book.MaSach.ToString());

						decimal tienDaTra = CustomerRefundBillBUS.Instance.getRefundRevenueByBookId(book.MaSach.ToString());

						dgvBook.Rows.Add(new object[] {
							book.MaSach,
							book.TenSach,
							book.HinhAnh.Length > 0 ? book.HinhAnh : defaultImg,
							daBan - daDoi - daTra,
							string.Format("{0:N0} VNĐ", doanhThu - tienDaDoi - tienDaTra)
						});
					}
				}
		...
		private void ProductStatisticGUI_Load(object sender, EventArgs e)
		{
			try
			{
				List<BookDTO> bookList = BookBUS.Instance.getAllDataThongKe();
		...
		private DataTable handleFilter(string query)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(revenueFrom.Text) && string.IsNullOrEmpty(revenueTo.Text))
				{
					...
					foreach (DataRow row in bookListDT.Rows)
					{
						if (Convert.ToDecimal(row["doanhThu"]) <= Convert.ToInt32(revenueFrom.Text))
						{
							rowsToRemove.Add(row);
						}
					}
				...
				if (string.IsNullOrEmpty(revenueFrom.Text) && !string.IsNullOrEmpty(revenueTo.Text))
				{
					...
					foreach (DataRow row in bookListDT.Rows)
					{
						if (Convert.ToDecimal(row["doanhThu"]) >= Convert.ToInt32(revenueTo.Text))
						{
							rowsToRemove.Add(row);
						}
					}
				...
				if (!string.IsNullOrEmpty(revenueFrom.Text) && !string.IsNullOrEmpty(revenueTo.Text))
				{
					if (Convert.ToInt32(revenueFrom.Text) <= Convert.ToInt32(revenueTo.Text))
					{
						...
						foreach (DataRow row in bookListDT.Rows)
						{
							if (!(Convert.ToDecimal(row["doanhThu"]) >= Convert.ToInt32(revenueFrom.Text) && Convert.ToDecimal(row["doanhThu"]) <= Convert.ToInt32(revenueTo.Text)))
							{
								rowsToRemove.Add(row);
							}
						}
