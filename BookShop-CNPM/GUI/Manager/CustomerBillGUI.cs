        ...
        private void viewMoreBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if(this.dgvCustomerBill.CurrentCell != null)
                {
					...
					CustomerBillDTO customerBill = CustomerBillBUS.Instance.getById(row.Cells[0].Value.ToString());
        ...
        private void printPdfBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if(this.dgvCustomerBill.CurrentCell != null) 
                {
					...
					using (CustomerBillPrintForm customerBillPrintForm = new CustomerBillPrintForm(Convert.ToInt32(row.Cells[0].Value)))
					{
