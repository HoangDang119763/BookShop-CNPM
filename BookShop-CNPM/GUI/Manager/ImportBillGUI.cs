            ...
            try
            {
                ...
                if (dlgResult == DialogResult.Yes)
                {
                    foreach (DataGridViewRow row in this.dgvImportBill.Rows)
                    {
                        ...
                        {
                            ImportBillBUS.Instance.delete(row.Cells[0].Value.ToString());
                        }
        ...
        private void printPdfBtn_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.dgvImportBill.CurrentCell != null)
                {
					...
					using (ImportBillPrintForm importBillPrintForm = new ImportBillPrintForm(Convert.ToInt32(row.Cells[0].Value)))
					{
