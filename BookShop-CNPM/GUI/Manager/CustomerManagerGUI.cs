	...
	public partial class CustomerManagerGUI : Form
    {
        ...
        private void deleteBtn_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!isHaveSelect)
                {
                    MessageBox.Show("Bạn chưa chọn những khách hàng cần xóa");
                    return;
                }

                DialogResult dlgResult = MessageBox.Show(
                    "Bạn chắc chắn muốn xóa các khách hàng đã chọn chứ?",
                    "Xác nhận",
                    MessageBoxButtons.YesNo,
                ...
                if (dlgResult == DialogResult.Yes)
                {
                    foreach (DataGridViewRow row in this.dgvCustomer.Rows)
                    {
                        ...
                        {
                            CustomerBUS.Instance.delete(row.Cells[1].Value.ToString());
                        }

                    }
                    List<CustomerDTO> positionList = CustomerBUS.Instance.Search(this.searchInput.Text.Trim());
                    this.loadCustomerListToDataGridView(positionList);
