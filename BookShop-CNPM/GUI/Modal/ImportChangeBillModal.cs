...
namespace BookShop_CNPM.GUI.Modal
{
    public partial class ImportChangeBillModal : Form
    {
        ...
        public ImportChangeBillModal(int staffId, string title = "Tạo đơn nhập hàng thay đổi")
        {
            InitializeComponent();
            this.Text = title;
        ...
        private void ImportChangeBillModal_Load(object sender, EventArgs e)
        {
            ...
            if (importChangeBill != null)
            {
                ...
                this.staffTxt.Text = StaffBUS.Instance.getById(importChangeBill.MaNhanVien.ToString()).Ten;
                    ...
                    for (int i = 0; i < this.changeBookContainer.Controls.Count; i++)
                    {
                        ...
                        for (int j = 0; j < this.listBookContainer.Controls.Count; j++)
                        {
                            if (i == j)
                            {
                                ...
                                ImportChangeBillBUS.Instance.createImportChangeBillDetail(item, importChangeBillBill.MaDonNhapHang.ToString());
                            }
                        }
                    }
            ...
            try
            {
                ...
                if (this.importBookContainer.Controls.Count > 0)
                {
                    foreach (ImportBookControl control in importBookContainer.Controls)
                    {
                        if (control.check)
                        {
                            Console.WriteLine(control.getSoLuong());
                            ...
                            else
                            {
                                ...
                                ImportChangeBillDetailDTO importRefundBillDetail = new ImportChangeBillDetailDTO(0, book.MaSach,0,0,0, 0);
                ...
                if (list.Count > 0)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (!bookControls.Any(a => a.getId() == book.MaSach))
                        {
                            ...
                            control.details(book, importBillDetail.SoLuong - importBillDetail.SoLuongDoiTra, importBillDetail.DonGia,1);
                ...
                if (list.Count > 0)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (!bookControls.Any(a => a.getId() == book.MaSach))
                        {
                            ...
                            ImportBookControl control = new ImportBookControl();
                            control.details(book, importBillDetail.soLuong,0,importBillDetail.donGiaSachCanDoi);
                            this.changeBookContainer.Controls.Add(control);
                        };
                    }
                }
        ...
        private void BookInput_TextChanged(object sender, EventArgs e)
        {
            try
            {
                if (BookInput.Text != "")
                {
                    ...
                    {
                        List<BookDTO> Booklist = BookBUS.Instance.getBookListImport(this.BookInput.Text);
                        if (Booklist != null)
                        {
        ...
        private void CaculateTotalMoney()
        {
            try
            {
                ...
                {
                    this.totalMoneylbl.Text = "Trả thêm";
