...
namespace BookShop_CNPM.GUI.Modal
{
    public partial class CustomerChangeBillModal : Form
    {
        ...
        public CustomerChangeBillModal(int staffId, string title = "Tạo đơn bán hàng thay đổi")
        {
            InitializeComponent();
            this.Text = title;
        ...
        private void CustomerChangeBillModal_Load(object sender, EventArgs e)
        {
            ...
            ChangeBookControl.OnChange = CaculateTotalMoney;
            CartProductImportControl.OnChange = CaculateTotalMoney;
            ...
            {
                label1.Text = "Xem Chi Tiết Đơn Bán Hàng Thay Đổi";
                ...
                this.staffTxt.Text = StaffBUS.Instance.getById(customerChangeBill.MaNhanVien.ToString()).Ten;
                    ...
                    for (int i = 0; i < this.changeBookContainer.Controls.Count; i++)
                    {
                        ...
                        for (int j = 0; j < this.listBookContainer.Controls.Count; j++)
                        {
                            if (i == j)
                            {
                                ...
                                CustomerChangeBillBUS.Instance.createCustomerChangeBillDetail(item, newCustomerChangeBill.MaDonKhachHang.ToString());
                            }
                        }
                    }
            ...
            try
            {
                ...
                if (customerBillDetail != null)
                {
                    foreach (CustomerBillDetailDTO customerBillDetailDTO in customerBillDetail)
                    {
                        ...
                        control.details(book, customerBillDetailDTO.SoLuong, customerBillDetailDTO.SoLuongDoiTra, customerBillDetailDTO.DonGia);
                    ...
                    foreach (ImportBookControl control in customerBookContainer.Controls)
                    {
                        if (control.check)
                        {
                            ...
                            else
                            {
                                ...
                                CustomerChangeBillDetailDTO importRefundBillDetail = new CustomerChangeBillDetailDTO(0, book.MaSach,0,0,0,0);
                ...
                if (list.Count > 0)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (!bookControls.Any(a => a.getId() == book.MaSach))
                        {
                            ...
                            control.details(book, customerBillDetail.SoLuong - customerBillDetail.SoLuongDoiTra, customerBillDetail.DonGia, 0);
                ...
                if (list.Count > 0)
                {
                    foreach (var item in list)
                    {
                        ...
                        if (!bookControls.Any(a => a.getId() == book.MaSach))
                        {
                            ...
                            ImportBookControl control = new ImportBookControl();
                            control.details(book, customerBillDetail.soLuong,0, customerBillDetail.donGiaSachCanDoi);
                            this.changeBookContainer.Controls.Add(control);
                        };
                    }
                }
            ...
            try
            {
                if (list.Count > 0)
                {
                    ...
                    for (int i = 0; i < this.changeBookContainer.Controls.Count; i++)
                    {
                        ...
                        if (i == this.listBookContainer.Controls.Count)
                        {
                            if (lastControl.getPrice() == gia2 || lastControl.getId() == id)
                            {
                                foreach (var item in list)
                                {
                                    ...
                                    if (!bookControls.Any(a => a.getId() == book.MaSach))
                                    {
                                        ...
                                        control.details(book, lastControl.GetBookAmount(),lastControl.getPrice(), lastControl.getId());
                ...
                if (list.Count > 0)
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        if (!bookControls.Any(a => a.getId() == book.MaSach))
                        {
                            ...
                            ImportBookControl control = new ImportBookControl();
                            control.details(book, importBillDetail.soLuong,0, importBillDetail.donGiaSachMuonDoi);
                            this.listBookContainer.Controls.Add(control);
                            CaculateTotalMoney1();
                        };
                    }
        ...
        private void CaculateTotalMoney()
        {
            try
            {
                decimal totalMoneyRefund = 0;
                decimal totalMoneyChange = 0;
                if (changeBookContainer.Controls.Count > 0)
                {
                    foreach (ChangeBookControl control in changeBookContainer.Controls)
                    {
                        int sl = control.GetBookAmount();
                        totalMoneyRefund += control.getPrice() * sl;
                    }
                    this.totalMoneyRefund.Text = totalMoneyRefund.ToString();
                }
                else
                {
                    this.totalMoneyRefund.Text = totalMoneyRefund.ToString();
                }
                if (listBookContainer.Controls.Count > 0)
                {

                    foreach (CartProductImportControl control in listBookContainer.Controls)
                    {
                        int sl = control.GetBookAmount();
                        totalMoneyChange += control.GetPrice() * sl;
                    }
                    this.totalMoneyChange.Text = totalMoneyChange.ToString();
                }
                else
                {
                    this.totalMoneyChange.Text = totalMoneyChange.ToString();
                }
                if ((totalMoneyRefund - totalMoneyChange)>0)
                {
                    this.totalMoneylbl.Text = "Tiền thừa";
                    this.totalMoney.Text = (totalMoneyRefund - totalMoneyChange).ToString();
                }
                else
                {
                    this.totalMoneylbl.Text = "Khách trả thêm";
                    this.totalMoney.Text = Math.Abs(totalMoneyRefund - totalMoneyChange).ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void CaculateTotalMoney1()
        {
            try
            {
                decimal totalMoneyRefund = 0;
                ...
                if (changeBookContainer.Controls.Count > 0)
                {
                    foreach (ImportBookControl control in changeBookContainer.Controls)
                    {
                        int sl = control.getSoLuong();
                        totalMoneyRefund += control.getPrice() * sl;
                    }
                    this.totalMoneyRefund.Text = totalMoneyRefund.ToString();
                }
                ...
                if (listBookContainer.Controls.Count > 0)
                {
                    ...
                    foreach (ImportBookControl control in listBookContainer.Controls)
                    {
                        int sl = control.getSoLuong();
                        totalMoneyChange += control.getPrice() * sl;
                    }
