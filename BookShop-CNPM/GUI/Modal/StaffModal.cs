    ...
    public partial class StaffModal : Form
    {
        ...
        private void StaffModal_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (authDetails != null )
                {
                    if(!authDetails.Any(c=>c.maQuyenHan == 13 && c.TrangThai))
                    {
        ...
        private void loadPositionCbx()
        {
            try
            {
                ...
                List<PositionDTO> filteredPositions = positions
         .Where(p => p.MaChucVu != 1)
         .ToList();
                filteredPositions.Insert(0, new PositionDTO(0, "Chọn chức vụ", "", false, 0));
                this.positionCbx.DisplayMember = "TenChucVu";
                this.positionCbx.ValueMember = "MaChucVu";
                this.positionCbx.DataSource = filteredPositions;
